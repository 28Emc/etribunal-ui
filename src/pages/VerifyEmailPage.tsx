...
import { Gavel, CheckCircle, X, AlertCircle } from 'lucide-react';
...
export function VerifyEmailPage() {
  ...
  const successView = (
    ...
    >
      ...
      <p className="text-text-muted text-sm mb-2">{t('auth.emailVerifiedMessage')}</p>
      <p className="text-text-muted text-xs mb-8">{t('auth.redirectingToLogin')}</p>
    </motion.div>
  );

  let view: React.ReactNode;
  if (loading) view = loadingView;
  else if (error) view = errorView;
  else view = successView;
